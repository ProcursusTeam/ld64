...
mkpath_np(const char *path, mode_t omode)
{
	...
	if (0 == mkdirat(AT_FDCWD, path, omode)) {
		goto mkpath_exit;
	}
	...
	while (1) {
		...
		*s = '\0';
		...
		if (0 == mkdirat(AT_FDCWD, apath, S_IRWXU | S_IRWXG | S_IRWXO)) {
			...
			struct stat dirstat;
			if (-1 == fstatat(AT_FDCWD, apath, &dirstat, 0)) {
			...
			if ((dirstat.st_mode & (S_IWUSR | S_IXUSR)) != (S_IWUSR | S_IXUSR)) {
				...
				if (-1 == fchmodat(AT_FDCWD, apath, chmod_mode, 0)) {
					/* Really unfortunate timing ... */
					retval = ENOENT;
					goto mkpath_exit;
				}
			 ...
			 */
			if (fstatat(AT_FDCWD, apath, &sbuf, 0) == 0 &&
	...
	while (depth > 1) {
		...
		*s = '/';
		...
		if (-1 == mkdirat(AT_FDCWD, apath, S_IRWXU | S_IRWXG | S_IRWXO)) {
	...
	if (-1 == mkdirat(AT_FDCWD, path, omode)) {
		retval = errno;
		if (errno == EEXIST &&
		    fstatat(AT_FDCWD, path, &sbuf, 0) == 0 &&
		    !S_ISDIR(sbuf.st_mode)) {
			retval = ENOTDIR;
		}
	}
